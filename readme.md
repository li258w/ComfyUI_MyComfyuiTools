...
* **分类系统**：
    ...
    * **Background Selector**：2层简化分类（环境基调 + 场景类型）
...
2.  **Clothing Selector** (`clothing.json`) - 服装
    * **特别功能**：支持12个功能分类，双级下拉菜单界面
    * **分类内随机**：可选择"*全部* (随机)"在特定分类内随机选择
    * **分类列表**：上装（日常）、连衣裙（日常）、套装、内衣 / 泳装、民族服装、晚宴红毯、正式商务、运动休闲、街头、朋克、海滩度假、居家休闲、职业装
3.  **Pose Selector** (`pose.json`) - 姿势
    * **特别功能**：支持13个姿势类型分类，双级下拉菜单界面
    * **分类内随机**：可选择"*全部* (随机)"在特定分类内随机选择
    * **分类列表**：站姿、坐姿、蹲跪姿、手势动作、表情情绪、正面朝向、背面朝向、侧面转身、运动动态、互动姿势、日常休闲、正式姿势、其他
...
📅 更新日志 (Changelog)
[2026-04-14] v2.1 - Background Category System
新增功能：为 Background Selector 节点添加背景分类系统和双级下拉菜单。

**简化分类系统**：
- 采用极度简化的2层标签分类：环境基调（室内/室外） + 场景类型（科幻/奇幻/日常/生活等）
- 设计理念：极度简化、聚焦核心、高效率分组，用两个标签快速定位场景的物理位置和核心主题
- 为所有425个背景条目添加分类标签，支持多标签

**双级下拉菜单**：
- 第一级：选择背景分类（category_select）
- 第二级：选择具体背景项目（item_select），根据第一级选择动态过滤
- 支持分类内随机选择：在 item_select 中添加 "*全部* (随机)" 选项，可在所选分类内随机选择项目

**技术实现**：
- 前端：复用现有 JavaScript 联动逻辑（js/widgets.js），扩展支持Background节点
- 后端：Python 处理分类映射和随机逻辑，与服装、姿势、构图、体型选择器共享架构
...
**自动分类系统**：
- 为所有姿势条目添加分类标签，支持13个姿势类型分类
- 分类包括：站姿、坐姿、蹲跪姿、手势动作、表情情绪、正面朝向、背面朝向、侧面转身、运动动态、互动姿势、日常休闲、正式姿势、其他
...
**自动分类系统**：
- 使用 AI 辅助脚本将 430 个服装条目自动分类到 12 个功能分类
- 分类包括：上装（日常）、连衣裙（日常）、套装、内衣 / 泳装、民族服装、晚宴红毯、正式商务、运动休闲、街头、朋克、海滩度假、居家休闲
